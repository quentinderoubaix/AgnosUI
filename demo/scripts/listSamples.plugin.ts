...
export const getSamplesList = () => {
	...
	const add = (style: SampleInfo['style']) => (file: string) => {
		const parts = file.slice(0, -13).split(path.sep);
		if (parts.length !== 2) {
	...
	globSync('**/+page.svelte', {cwd: path.join(import.meta.dirname, '../../svelte/demo/src/routes/bootstrap')}).forEach(add('bootstrap'));
	globSync('**/+page.svelte', {cwd: path.join(import.meta.dirname, '../../svelte/demo/src/routes/daisyui')}).forEach(add('daisyui'));
	return map;
};
