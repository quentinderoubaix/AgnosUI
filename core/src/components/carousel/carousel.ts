...
import {createTypeEnum, typeArray, typeBoolean, typeNumber, typeString, typeStringOrNull} from '../../utils/writables';
...
/**
 * Represents the API for a carousel slide.
 */
export interface CarouselSlideApi {
	 ...
	 * @param jump - scroll instantly
	 */
	select: (jump?: boolean) => void;
}

/**
 * Represents a carousel slide widget with specific properties, state, API, and directives.
 */
export type CarouselSlideWidget = Widget<CarouselSlideProps, CarouselSlideState, CarouselSlideApi, CarouselSlideDirectives>;
 ...
 * @param select - function to call to select the slide
 * @param selectedId$ - the id of the selected carousel slide
 * @param config - an optional carousel slide config
 * @returns a CarouselSlideWidget
 */
export function createCarouselSlide(
	select: (id: string) => void,
	selectedId$: ReadableSignal<string>,
		...
		api: {
			...
			registerSlide: (config?: PropsConfig<CarouselSlideProps>) => {
				const slide = createCarouselSlide(
					(id: string, jump?: boolean) =>
						emblaCarousel.api.scrollTo(
				...
				slide.directives.slideDirective = mergeDirectives(
					...
					slide.directives.slideDirective,
				);
				return slide;
			},
		},
